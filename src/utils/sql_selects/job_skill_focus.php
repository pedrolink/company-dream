<?php 
if($row_edit_job['name_carrer']){
    $sql_select_carrers_focus = 'SELECT * FROM carrers_focus WHERE id_carrer != "' . $row_edit_job['id_carrer'] . '"';
    $result_select_carrers_focus = mysqli_query($conection, $sql_select_carrers_focus);
    echo '<option value=' . $row_edit_job['id_carrer'] . '>' . $row_edit_job['name_carrer'] . '</option>';
